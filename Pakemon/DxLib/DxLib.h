...
// ＤＸライブラリのバージョン
#define DXLIB_VERSION 0x311b
#define DXLIB_VERSION_STR _T( "3.11b" )
...
extern	int			SetUseGPUShaderModel3(						int Flag ) ;									// GPU のプログラマブルシェーダーモデル3.0が使用できる環境では優先的にシェーダーモデル3.0をしようするかどうかを設定する( TRUE:優先的に使用する  FALSE:必要な場合のみ使用する( デフォルト ) )
